...
class test_1(TaskSet):
    ...
    def test_api(self):
        ...
        # r是包含所有响应内容的一个对象
        r = self.client.get("/idGetRarity", timeout=30, headers=header)
...
class test_2(TaskSet):
    ...
    def test_api(self):
        ...
        header = {"User-Agent": "Mozilla/5.0 "
                                ...
                                "(KHTML, like Gecko) Chrome/60.0.3112.101 Safari/537.36"}
        # r是包含所有响应内容的一个对象
        r = self.client.get("/idGetCombatPoints", timeout=30, headers=header)
        # 这里可以使用assert断言请求是否正确，也可以使用if判断
        if r.status_code == 200:
            print("success")

class test_3(TaskSet):
    # task装饰该方法为一个事务方法的参数用于指定该行为的执行权重。参数越大，每次被虚拟用户执行概率越高，不设置默认是1，
    @task
    def test_api(self):
        # 定义requests的请求头
        header = {"User-Agent": "Mozilla/5.0 "
                                ...
                                "(KHTML, like Gecko) Chrome/60.0.3112.101 Safari/537.36"}
        # r是包含所有响应内容的一个对象
        r = self.client.get("/getSoldiersJson", timeout=30, headers=header)
        # 这里可以使用assert断言请求是否正确，也可以使用if判断
        if r.status_code == 200:
            print("success")
...
if __name__ == '__main__':
    ...
    os.system('locust -f pressure.py --host=http://127.0.0.1:8000/')
