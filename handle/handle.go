...
func HandleIdGetCombatPoints(id string)map[string]string{
	...
	retMap := make(map[string]string,3)
	if len(id) != 0 {
		ret := service.GetCombatPoints(id,*UMAP)
		if  ret != ""{
			retMap["condition"]="success"
			retMap["id"]=id
...
func HandleGetStage(stage string)(map[string]string,[] *soldierInfo.Info){
	...
	if len(stage) != 0 {
		dataRes  = service.GetSoldiersJson(stage ,*UMAP)
		if dataRes != nil{
...
func HandleGetArmyAssembled(rarity string , stage string ,cvc string, Umap map[string]*soldierInfo.Info) (data []*soldierInfo.Info,conditon string){
	return service.GetArmyAssembled(rarity,stage,cvc,*UMAP)
}
